<template>
  ...
  const init = async (start: string, end: string) => {
    const res = await getOrderSourceData({
      projectId: 106,
      fieldId: 14,
      dtTo: new Date(end).toLocaleDateString(),
      dtFrom: new Date(start).toLocaleDateString(),
    });
    let workDataList: workDataInfo[] = [];
    res.forEach((item)=>{
        if(item.name)  workDataList.push(item);                      
    })
    
    yAxisData = workDataList.map((item) =>item.name).reverse();
    seriesData = workDataList.map((item) => item.count).reverse();
