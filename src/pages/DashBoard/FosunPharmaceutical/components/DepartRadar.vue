...
import { getIdNameCountGroupByCustomerField } from '@/api/dashboard/fosun';
...
const init = async ({ start, end }: { start: string; end: string }) => {
  const res = await getIdNameCountGroupByCustomerField({
    projectId: 106,
    fieldId: 10001,
  ...
  indicator = res.map((item) => ({ name: item.name }));
